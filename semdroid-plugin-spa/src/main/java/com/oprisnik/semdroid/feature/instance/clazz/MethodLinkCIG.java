...
/**
 * Class instance generator that creates links to all methods of the class.
 */
public class MethodLinkCIG extends ClassInstanceGenerator {

    @Override
    protected void getClassInstance(DexClass clazz, InstanceDataElement results) {
        // add dex method references
        for (DexMethod m : clazz.getMethods()) {
            results.addValue(getInstanceReferenceDataElement(m));
        }
    }
