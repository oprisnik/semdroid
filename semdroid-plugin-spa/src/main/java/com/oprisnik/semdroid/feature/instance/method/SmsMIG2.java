...
/**
 * Method instance generator for SMS plugins.
 */
public class SmsMIG2 extends MethodInstanceGenerator {


    private static final String TAG = "SmsMIG2";
...
//                histogram));
        ...
        for (LocalVariable l : method.getLocalVariables(getMethodCallInclusionDepth())) {
            ...
            if (type != null
                    ...
                    .dataTypeStartsWith(type, "android.telephony"))) {
                localVarSet.add(type);
            }
        }
        for (MethodCall mc : method.getMethodCalls(getMethodCallInclusionDepth())) {
            if (mc.getClassName().startsWith("android.telephony")) {
