...
/**
 * Method instance generator that uses opcodes.
 */
public class OpcodeMIG extends MethodInstanceGenerator {

    @Override
    protected void getMethodInstance(DexMethod method,
                                     InstanceDataElement results) {
        for (Opcode op : method.getOpcodes()) {
            results.addValue(getSymbolicFeatureDataElement("opcode",
                    getOpcodeGroupName(op)));
        }
    }
