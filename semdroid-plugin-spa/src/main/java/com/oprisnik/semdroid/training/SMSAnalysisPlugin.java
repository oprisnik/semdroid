 ...
 */
public class SMSAnalysisPlugin extends BaseAnalysisPlugin {
    ...
    protected void labelMethod(AppAnalysisReport report, DexMethod m) {
        boolean catcherFound = false;
        for (MethodCall c : m.getMethodCalls()) {
            if (c.getName().contains("abortBroadcast")) {
                Log.d(TAG, "SMS_CATCHER: " + m.getMethodName());

                report.label(m, "SMS_CATCHER");
                catcherFound = true;
                break;
            }
        }
