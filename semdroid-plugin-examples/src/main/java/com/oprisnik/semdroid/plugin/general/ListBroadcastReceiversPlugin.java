 ...
 */
public class ListBroadcastReceiversPlugin extends BaseAnalysisPlugin {

    @Override
    public void analyze(AppAnalysisReport report, App app) {
        for (DexClass c : app.getClasses()) {
            String ext = c.getFirstExternalSuperClass();
            if (ext != null && ext.contains("BroadcastReceiver")) {
                report.label(c, "BroadcastReceiver");
            }
        }
    }
}
